...
@export var card_asset_dir: String = "res://assets/cards"
...
func create_card_from_data(card_data: Dictionary, target: CardContainer) -> Card:
	...
	var front_image_path = card_asset_dir + "/" + card_data["front_image"]
	var front_texture = load(front_image_path) as Texture2D
	
	# Add to scene tree BEFORE setting faces (some cards need to be in tree)
	# Target might have a "Cards" child node based on framework
